...
def start_codex_if_needed(session: str, workdir: str, prompt: str = "") -> str:
    existing = tmux_capture(session)
    if "codex" in existing.lower() or "▣" in existing:
        return "reused"
    if prompt:
        # Use heredoc to safely pass multi-line prompt to codex exec via stdin
        # This avoids shell quoting issues with special chars
        safe_prompt = prompt.replace('"', '\\"')
        cmd = f'cd {workdir} && /usr/bin/codex exec "$(cat <<"CODEXPROMPT"\n{safe_prompt}\nCODEXPROMPT)"'
        tmux_send(session, cmd)
    else:
        tmux_send(session, f"cd {workdir} && /usr/bin/codex exec")
    return "started"
