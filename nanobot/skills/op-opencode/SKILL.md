---
name: op-opencode
description: 在本机通过 tmux 调用 opencode 执行任务，并按 start / monitor / continue / stop 标准流程管理任务
metadata: {"nanobot":{"emoji":"🌐","os":["linux","darwin"],"requires":{"bins":["tmux","opencode","python3"]}}}
---

# op-opencode Skill

在本机通过 tmux 启动并控制 opencode，用于执行可持续观察、可续推、可恢复的长任务。
...
该技能的目标是把 opencode 任务管理流程标准化：
1. 规划任务提示词
2. 在指定 tmux session 中运行 opencode
3. 使用 30 秒周期监控抓取输出
...
- `skills/op-opencode/runtime/<task_id>.json`
...
```bash
tmux new-session -d -s opencode-task
```

启动 opencode：

```bash
tmux send-keys -t opencode-task 'cd /home/work/nanobot && /root/.opencode/bin/opencode' C-m
```

发送初始提示词：

```bash
tmux send-keys -t opencode-task '<初始提示词>' C-m
```

### start 模式约束

- 若发现该 `task_id` 已存在有效监控任务，不得再创建新 cron
- 若 session 已存在且 opencode 正在运行，优先复用
- start 模式只在首次启动时使用
...
```bash
tmux capture-pane -pt opencode-task
```
...
用于在已有 tmux session 中向 opencode 继续发送提示词。
...
```bash
python3 /home/work/nanobot/nanobot/skills/op-opencode/start_task.py \
  --goal '<任务目标>' \
  --context '<上下文>' \
  --constraints '<约束>' \
  --output '<期望输出>' \
...
```bash
python3 /home/work/nanobot/nanobot/skills/op-opencode/monitor_tmux.py monitor <task_id>
```

人工续推：

```bash
python3 /home/work/nanobot/nanobot/skills/op-opencode/monitor_tmux.py continue <task_id> '<prompt>'
```

恢复已有任务的监控链路：

```bash
python3 /home/work/nanobot/nanobot/skills/op-opencode/resume_task.py <task_id> '<prompt>' --cron-job-id <job_id>
```

停止任务：

```bash
python3 /home/work/nanobot/nanobot/skills/op-opencode/stop_task.py <task_id> --status completed
```

如需同时关闭 session：

```bash
python3 /home/work/nanobot/nanobot/skills/op-opencode/stop_task.py <task_id> --status stopped --close-session
```
...
创建 `op-opencode` 监控 cron 时，建议使用如下模板，并替换其中的 `<TASK_ID>` 与 `<CHAT_ID>`：

```text
请运行 op-opencode 监控流程，检查 task_id=<TASK_ID> 对应的 tmux 会话输出，并执行 /home/work/nanobot/nanobot/skills/op-opencode/monitor_tmux.py monitor <TASK_ID>。
...
当用户要求使用 `op-opencode` 时，必须先判断当前应该进入哪种模式：
